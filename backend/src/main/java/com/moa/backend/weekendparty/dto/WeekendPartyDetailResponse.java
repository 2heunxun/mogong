...
@Schema(description = "주말팟 상세 정보")
public record WeekendPartyDetailResponse(
        @Schema(description = "주말팟 ID") Long id,
        @Schema(description = "주말팟 제목") String title,
        @Schema(description = "주말팟 소개", nullable = true) String description,
        ...
        @Schema(description = "요청자 본인이 이 주말팟의 파티장인지 여부. 비로그인 요청이면 항상 false.")
        boolean isOwner,
        ...
        @Schema(description = "주말팟 생성 시각") LocalDateTime createdAt,
        @Schema(description = "주말팟 정보 마지막 수정 시각") LocalDateTime updatedAt
) {
    public static WeekendPartyDetailResponse of(WeekendParty party, long memberCount, boolean isOwner, WeekendPartyMember.Status memberStatus) {
        boolean canSeeOpenChat = isOwner || memberStatus == WeekendPartyMember.Status.APPROVED;
        return new WeekendPartyDetailResponse(
                party.getId(),
                party.getTitle(),
