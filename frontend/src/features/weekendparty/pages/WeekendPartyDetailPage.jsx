...
import {
  approveWeekendPartyJoinRequest,
  deleteWeekendParty,
  fetchWeekendPartyDetail,
  fetchWeekendPartyJoinRequests,
  fetchWeekendPartyMembers,
  joinWeekendParty,
  leaveWeekendParty,
  rejectWeekendPartyJoinRequest,
} from '../api/weekendParties';
import { useAuth } from '../../auth/hooks/useAuth';

export default function WeekendPartyDetailPage() {
  const { id } = useParams();
  ...
  const load = () => {
    setLoading(true);
    setError('');
    Promise.all([fetchWeekendPartyDetail(id), fetchWeekendPartyMembers(id)])
      .then(([detail, memberList]) => {
        setParty(detail);
        setMembers(memberList);
        if (detail.isOwner) {
          return fetchWeekendPartyJoinRequests(id).then(setJoinRequests);
        }
        setJoinRequests([]);
      })
  ...
  const handleLeave = async () => {
    if (!window.confirm('주말팟에서 탈퇴하시겠습니까?')) return;
    setActionError('');
    setActionLoading(true);
    try {
      await leaveWeekendParty(id);
  ...
  const handleDelete = async () => {
    if (!window.confirm('주말팟을 삭제하시겠습니까? 되돌릴 수 없습니다.')) return;
    setActionLoading(true);
    try {
      await deleteWeekendParty(id);
      navigate('/weekend-parties');
    } catch (err) {
  ...
  const handleApproveRequest = async (memberId) => {
    ...
    setActionLoading(true);
    try {
      await approveWeekendPartyJoinRequest(id, memberId);
      load();
    } catch (err) {
  ...
  const handleRejectRequest = async (memberId) => {
    ...
    setActionLoading(true);
    try {
      await rejectWeekendPartyJoinRequest(id, memberId);
      load();
    } catch (err) {
    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mb-8 flex flex-wrap items-center gap-2">
        ...
        {party.isOwner && (
          <>
            <Link
              to={`/weekend-parties/${party.id}/edit`}
