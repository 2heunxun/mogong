...
import { fetchWeekendParties } from '../api/weekendParties';
import WeekendPartyCard from '../components/WeekendPartyCard';
import { CATEGORIES } from '../constants/categories';

export default function WeekendPartyListPage() {
  ...
  useEffect(() => {
    ...
    fetchWeekendParties({ category: category || undefined, status: status || undefined, keyword: keyword || undefined, page, size: 9 })
      .then((res) => {
        if (!ignore) setData(res);
      })
      .catch(() => {
        if (!ignore) setError('주말팟 목록을 불러오지 못했습니다.');
      })
      .finally(() => {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <h1 className="mb-6 text-2xl font-bold text-slate-900">주말팟 목록</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {data.content.map((party, index) => (
          <WeekendPartyCard key={party.id} party={party} index={index} />
        ))}
      </div>
